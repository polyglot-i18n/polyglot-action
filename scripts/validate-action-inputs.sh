...
if [ "$MANAGED_FIELDS" -ne 0 ] && [ "$MANAGED_FIELDS" -ne 3 ]; then
  echo "::error::managed-token, managed-run-id, and managed-policy-hash must be provided together" >&2
  exit 1
fi

if [ "$MANAGED_FIELDS" -eq 3 ]; then
